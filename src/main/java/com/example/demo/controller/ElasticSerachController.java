...
public class ElasticSerachController {
     ...
     */
    @ResponseBody
    @RequestMapping(value = "/es/api/person/save" ,method = RequestMethod.POST)
    public ResponseView saveEsPerson(@RequestBody Person person){
        ResponseView responseView = new ResponseView();
        person.setBirthday(DateUtil.parse("990-01-01 03:04:05"));
...
//        Person person = new Person(1L, "刘备", "蜀国", 18, DateUtil.parse("1990-01-02 03:04:05"), "刘备（161年－223年6月10日），即汉昭烈帝（221年－223年在位），又称先主，字玄德，东汉末年幽州涿郡涿县（今河北省涿州市）人，西汉中山靖王刘胜之后，三国时期蜀汉开国皇帝、政治家。\n刘备少年时拜卢植为师；早年颠沛流离，备尝艰辛，投靠过多个诸侯，曾参与镇压黄巾起义。先后率军救援北海相孔融、徐州牧陶谦等。陶谦病亡后，将徐州让与刘备。赤壁之战时，刘备与孙权联盟击败曹操，趁势夺取荆州。而后进取益州。于章武元年（221年）在成都称帝，国号汉，史称蜀或蜀汉。《三国志》评刘备的机权干略不及曹操，但其弘毅宽厚，知人待士，百折不挠，终成帝业。刘备也称自己做事“每与操反，事乃成尔”。\n章武三年（223年），刘备病逝于白帝城，终年六十三岁，谥号昭烈皇帝，庙号烈祖，葬惠陵。后世有众多文艺作品以其为主角，在成都武侯祠有昭烈庙为纪念。");
     ...
     */
//    @ResponseBody
//    @RequestMapping(value = "/es/api/person/saveAll" ,method = RequestMethod.GET)
//    public ResponseView saveAllEsPerson(@RequestParam String remark){
//        ResponseView responseView = new ResponseView();
...
//    }
    ...
    @ResponseBody
    @RequestMapping(value = "/es/api/name/get/termQuery" ,method = RequestMethod.GET)
    public ResponseView getEsNameForTermQuery(@RequestParam String remark){
        ResponseView responseView = new ResponseView();

        // QueryBuilders 提供了很多静态方法，可以实现大部分查询条件的封装
        TermQueryBuilder queryBuilder = QueryBuilders.termQuery("remark", remark);
        ArrayList<Person> personArrayList = new ArrayList<>();
        repo.search(queryBuilder).forEach(person -> personArrayList.add(person));
    ...
    @RequestMapping(value = "/es/api/name/get/multiMatchQuery" ,method = RequestMethod.GET)
    public ResponseView getEsNameForMultiMatchQuery(@RequestParam String remark) {
        ResponseView responseView = new ResponseView();

        // QueryBuilders 提供了很多静态方法，可以实现大部分查询条件的封装
        MultiMatchQueryBuilder queryBuilder = QueryBuilders.multiMatchQuery( remark,"name","id","age","remark");
        ArrayList<Person> personArrayList = new ArrayList<>();
        repo.search(queryBuilder).forEach(person -> personArrayList.add(person));
    ...
    @RequestMapping(value = "/es/api/name/get/fuzzyQuery" ,method = RequestMethod.GET)
    public ResponseView getEsNameForFuzzyQuery(@RequestParam String remark) {
        ResponseView responseView = new ResponseView();

        // QueryBuilders 提供了很多静态方法，可以实现大部分查询条件的封装
        FuzzyQueryBuilder queryBuilder = QueryBuilders.fuzzyQuery("remark", remark);
        ArrayList<Person> personArrayList = new ArrayList<>();
        repo.search(queryBuilder).forEach(person -> personArrayList.add(person));
    ...
    @ApiParam(required = true, name = "test", value = "教程入参")
    @RequestMapping(value = "/es/api/name/get/matchPhraseQuery" ,method = RequestMethod.GET)
    public ResponseView getEsNameForMatchPhraseQuery(@RequestParam String remark) {
        ResponseView responseView = new ResponseView();

        // QueryBuilders 提供了很多静态方法，可以实现大部分查询条件的封装
        MatchPhraseQueryBuilder queryBuilder = QueryBuilders.matchPhraseQuery("remark", remark);
        ArrayList<Person> personArrayList = new ArrayList<>();
        repo.search(queryBuilder).forEach(person -> personArrayList.add(person));
